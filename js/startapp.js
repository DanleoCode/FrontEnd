...
startApp.config([ '$stateProvider', '$urlRouterProvider',
		'RestangularProvider',
...
startApp.controller("loginController", [
										...
										'$cookies',
										'$cookieStore',
		function($scope, Restangular, $window, $cookies, $cookieStore) {
			...
			$scope.changePage = function() {
				...
				Restangular.all("auth/login").post(
						$scope.loginCredential).then(function(data) {
					$scope.userInfo = data.plain();
//					console.log($scope.userInfo);
					if (Number($scope.userInfo.empId) > 0) {
						var obj = {
							currentUser : $scope.userInfo
						};
				...
				// Restangular.all("leavemanagement/api/auth/logout").post().then(function(data){
...
startApp.controller('RegistrationController',['$scope', 'Restangular', function($scope, Restangular){
	$scope.employeeInfo = {};
	
	$scope.createNewUser = function(){
		
		Restangular.all("employee").post($scope.employeeInfo)
			.then(function(data){
				var res = data.plain();
